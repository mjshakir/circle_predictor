...
int main(int argc, char const *argv[]){
    ...
    auto _circle_reward = [&_normalize](const torch::Tensor& input, const torch::Tensor& output) {
        ...
        auto R_consistency_reward = threadPool.queue(&Utils::CircleEquation::consistency_reward,_output);
         
        // Add penalties for points outside the circle
        auto R_distance_penalty = threadPool.queue(&Utils::CircleEquation::distance_penalty, _input, _output);
        
        // Compute separation penalty
        constexpr double min_distance = 1E-1;  // You can adjust this value as needed
        auto R_separation_penalty = threadPool.queue(&Utils::CircleEquation::separation_penalty, _output, min_distance);
