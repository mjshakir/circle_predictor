...
struct LSTMNet : torch::nn::Module {
  ...
  public:
    ...
    LSTMNet(torch::Device& device): m_device(device),
                                    recurrent_layer(torch::nn::LSTMOptions(10, 20).num_layers(64).batch_first(true).bidirectional(true).dropout(0.5)),
  ...
  protected:
    ...
    torch::Tensor lstm_layers(torch::Tensor& x){
      ...
      //--------------------------
      // std::cout << "_input_lstm: " << _input_lstm.sizes() << std::endl;
      //--------------------------
      ...
      //-------------------------
      // std::cout << "std::get<0>(x_lstm): " << std::get<0>(x_lstm).sizes() << std::endl;
      //--------------------------
      return std::get<0>(x_lstm).view(-1);
      //-------------------------
      // return std::get<0>(x_lstm).view();
      //-------------------------
  ...
  private:
    ...
    torch::nn::Linear output_layer;
    //--------------------------
    torch::Tensor h0 = torch::from_blob(std::vector<float>(1*64*2, 0.0).data(), {64*2, 2, 20});
    torch::Tensor c0 = torch::from_blob(std::vector<float>(1*64*2, 0.0).data(), {64*2, 2, 20});
    //--------------------------
